...
MODELS = {
    "tinyllama": {
        ...
        # tensormark/data is where build_llama.sh's binaries, tmchat.py and the
        # grammar gates all look by default — and the path the gates print in
        # their "run examples/fetch_model.py" skip message. Writing to the repo
        # root instead meant following that message did not fix the skip.
        "out": "tensormark/data/tinyllama",
        "size_gb": 2.2,
    },
}
...
def main() -> None:
    ...
    ap.add_argument("--tokenizer-only", action="store_true",
                    help="fetch the SentencePiece tokenizer and stop; the CI grammar "
                         "gates pair a real vocabulary with a generated tiny model")
    a = ap.parse_args()
    if a.list:
    ...
    if a.tokenizer_only:
        # The grammar mask iterates the tokenizer's pieces and each sampled id
        # indexes the model's embeddings, so the vocabulary must be real and must
        # match the model's vocab_size. The weights need not — see
        # tensormark/gen_tiny_llama.py.
        tok = out / m["tokenizer"]
        if not tok.exists():
    ...
    # tensormark/build is where build_llama.sh puts it and where the README says
    # to look; building into the repo root instead left the documented path empty.
    exe = REPO / "tensormark" / "build" / "convert_tmq"
