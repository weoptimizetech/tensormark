...
Sampling: `--oneshot` is greedy-deterministic (audit-friendly, reproducible), and
interactive and batch modes are greedy too — argmax is the CLI default, because on a 4B
checkpoint the sampler measurably loses the kind of task these models get asked to do.
`--sampling` turns temperature 0.7 / top-k 40 / top-p 0.9 back on, and `/greedy` toggles
between the two live. Context: 4096 by default — deliberately smaller than the box would
size for itself, so a large Q6_K pack does not put its KV cache into swap; `TM_CTX`
overrides it. The chat template is auto-detected
