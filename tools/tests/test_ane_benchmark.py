"""Input/rejection contracts for the ANE benchmark, using local weights.

The benchmark's source is retired: docs/retired/bench_ane_split.cpp drove the
in-engine ANE dispatch API that was removed from llama.h, so nothing builds it.
This suite still pins its input/rejection contracts for whoever rebuilds it —
building it into docs/retired/build/ re-enables the checks.
"""
import os
...
SOURCE = ROOT / "docs/retired/bench_ane_split.cpp"
BINARY = ROOT / "docs/retired/build/bench_ane_split"
MODEL = ROOT / "tensormark/data/tinyllama/tinyllama_q40.tmq"


@unittest.skipUnless(BINARY.exists() and MODEL.exists(),
                     f"build {SOURCE.relative_to(ROOT)} into {BINARY.relative_to(ROOT)} "
                     "and provide the TinyLlama TMQ")
