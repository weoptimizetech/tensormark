...
class AnepackTests(unittest.TestCase):
    ...
    def trace(self):
        ...
        with mock.patch.dict(sys.modules, {"coremltools": ct}):
            with contextlib.redirect_stdout(io.StringIO()):
                anepack.build(str(self.tmq), self.LAYERS, self.S, out)
        ...
        # The shim refuses any package whose output set is not h + k_<i>/v_<i>,
        # so the names ARE the contract, not a convenience.
        self.assertEqual([output.name for output in options["outputs"]],
                         ["h", "k_0", "v_0", "k_1", "v_1"])
    ...
    def test_named_kv_outputs_are_causal_and_carry_the_projected_kv(self):
        kv_graph, _ = self.trace()
        ...
        self.assertFalse(torch.equal(hidden[2:], changed_outputs[0][2:]))
    ...
    def key(self, **kwargs):
        options = dict(nlayers=self.LAYERS, tokens=self.S)
    ...
    def test_cache_identity_includes_source_config_and_build_options(self):
        ...
        for options in ({"nlayers": 1}, {"tokens": 8}):
    ...
    def fake_build(self, tmq, layers, tokens, out_pkg):
    ...
    def test_hidden_only_is_not_a_supported_lane(self):
        """The retired --no-kv flavour has no flag and no code path left."""
        with self.assertRaises(SystemExit):
            self.run_main("--no-kv")
        with mock.patch.object(anepack, "build", side_effect=self.fake_build) as build:
            self.run_main("--out", str(self.root / "kv.mlpackage"))
        self.assertNotIn("emit_kv", build.call_args.kwargs)
        self.assertNotIn("emit_kv", build.call_args.args)
