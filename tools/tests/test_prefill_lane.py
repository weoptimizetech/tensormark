...
The worker's source is retired: docs/retired/bench_prefill_lane.cpp drove the
in-engine ANE dispatch API that was removed from llama.h, so nothing builds it.
This suite still pins its protocol for whoever rebuilds it — building it into
docs/retired/build/ re-enables the checks.
"""
import json
import math
import os
...
SOURCE = ROOT / "docs/retired/bench_prefill_lane.cpp"
BINARY = ROOT / "docs/retired/build/bench_prefill_lane"
MODEL = ROOT / "tensormark/data/tinyllama/tinyllama_q40.tmq"


@unittest.skipUnless(BINARY.is_file() and MODEL.is_file(),
