...
Status: the ANE-lane worker it drives (docs/retired/build/bench_prefill_lane, from
docs/retired/bench_prefill_lane.cpp) no
longer compiles — it uses the in-engine ANE dispatch API that was removed from
llama.h. So this is the harness behind the admission-share study in docs/ANE.md,
kept as its record; its fake-worker tests still run.
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--binary", type=Path,
                        default=Path(__file__).resolve().parents[1] / "docs/retired/build/bench_prefill_lane")
