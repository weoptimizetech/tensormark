...
segment layer, so a consumer can ingest them into its KV cache and decode
normally through its own tail layers [K, L). Hidden-only (--no-kv) is an
experimental approximate lane: the final hidden state cannot reconstruct exact
per-layer K/V.
