...
The package emits hidden state plus post-RoPE K and V for every segment layer,
so a consumer can ingest them into its KV cache and decode normally through its
own tail layers [K, L). That is the only shape it builds: a hidden-only package
cannot reconstruct exact per-layer K/V, and the shim refuses it.

Status: the packer still builds, but nothing in this repository consumes its
output any more. The engine's ANE dispatch API (ane_ready / set_ane_enabled /
ane_prefills) was removed from llama.h when the lane concluded the ANE cannot add
to the hybrid, and no shipping binary reads TM_ANE=1. This tool and the record in
docs/ANE.md are what remain of the lane.

Usage:
  python3 tools/anepack.py <model.tmq> [--layers K] [--tokens T] [--out PKG] [--force]

Build cache: ~/.cache/tensormark/ane/<key>/ane_seg.mlpackage with key derived
from model name/size/mtime, K, T, builder source digest and the
...
def build(tmq_path, nlayers, S, out_pkg):
    ...
    # The shim's output contract: "h" plus "k_<i>"/"v_<i>" for every segment
    # layer, and nothing else — it refuses any other output set at load.
...
def cache_key(tmq_path, nlayers, tokens):
...
def main():
    ...
    key = cache_key(a.tmq, a.layers, a.tokens)
    if a.out:
    ...
    build(a.tmq, a.layers, a.tokens, out_pkg)
    with open(meta, "w") as f:
        json.dump({"package": out_pkg, "layers": a.layers, "tokens": a.tokens,
                   "cache_key": key, "model": os.path.basename(a.tmq)}, f, indent=1)
    print(out_pkg)
