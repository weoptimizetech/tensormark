...
class Engine:
    ...
    def __init__(self, model: Path, tokenizer: Path):
        ...
        # Greedy is the engine's default (21a8683) and /greedy is a TOGGLE, so the
        # blind send that used to select determinism now selects SAMPLING — the
        # opposite of what this gate needs. Flip until it reports the state we
        # require, then assert it, so this holds whichever default the engine ships.
        ack = self.command("/greedy")
        if "greedy=on" not in ack:
            ack = self.command("/greedy")
        assert "greedy=on" in ack, f"engine would not settle greedy: {ack!r}"
