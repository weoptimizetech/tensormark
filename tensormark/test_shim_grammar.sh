...
# 9b. A SYSTEM message survives the replay path. Ollama clients put the system
#     prompt in the message list and the shim replays the history turn by turn —
#     but the front-end's role check accepted only user/assistant, so every
#     request carrying a system message came back HTTP 500 with
#     "would not load the history". Nothing caught it: check 9 sends a single
#     user message (so no history to replay) and the interactive path takes its
#     system prompt from -s. This is the shape every coding agent sends.
SYS='{"messages":[{"role":"system","content":"You are terse."},{"role":"user","content":"Say hi."}],"budget":16}'
OUT=$(curl -s --max-time 240 -X POST "http://127.0.0.1:$PORT/api/chat" \
        -H 'Content-Type: application/json' -d "$SYS")
printf '%s' "$OUT" | python3 -c '
import json,sys
d=json.load(sys.stdin)
assert "error" not in d, f"system message rejected: {d}"
m=d.get("message") or {}
assert m.get("role")=="assistant", f"no assistant message: {d}"
print("  ok   /api/chat with a system message -> answered")' || fail "chat with system message: $OUT"

# 9c. A history turn too large for the context window is a CLIENT-visible limit, not
#     a server crash: HTTP 400 naming the knob. It used to be 500 "engine would not
#     load the history", which reads as a broken server and names neither the limit
#     nor how to raise it. The oversized text belongs in the HISTORY, not in the last
#     message — the last message goes through the turn loop, which shrinks its budget
#     instead of refusing.
LONG=$(python3 -c "print('the quick brown fox jumps over the lazy dog. ' * 900)")
OUT=$(curl -s -w '\n%{http_code}' --max-time 120 -X POST "http://127.0.0.1:$PORT/api/chat" \
        -H 'Content-Type: application/json' \
        -d "{\"messages\":[{\"role\":\"user\",\"content\":\"$LONG\"},{\"role\":\"user\",\"content\":\"Say hi.\"}],\"budget\":8}")
CODE=$(printf '%s' "$OUT" | tail -1)
BODY=$(printf '%s' "$OUT" | sed '$d')
if [ "$CODE" = "400" ] && printf '%s' "$BODY" | grep -q "context window"; then
  pass "oversized history turn -> HTTP 400 naming the context window"
else
  fail "oversized history turn returned $CODE (expected 400 + 'context window'): $(printf '%s' "$BODY" | head -c 200)"
fi
