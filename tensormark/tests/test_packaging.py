...
class SourceArchiveTests(unittest.TestCase):
    ...
    def test_native_local_include_closure(self):
        # All active archive includes must resolve inside the archive. The only
        # exception is the engine source's checkout fallback: headers that need
        # the engine's Tensor type include "../neural_demo.cpp", which is a real
        # file in a checkout (the repo root, one level above this package dir)
        # and reaches the archive as _native/neural_demo.cpp, staged by setup.py
        # and pulled in first by engine.h. The escape is allowed ONLY for the
        # files listed here, with that exact target, so a new header that points
        # outside the archive still fails this gate.
        CHECKOUT_FALLBACK = {
            "engine.h": "../neural_demo.cpp",
            "safetensors.h": "../neural_demo.cpp",
        }
        ...
        while pending:
            ...
            for include in re.findall(r'^\s*#\s*include\s*"([^"]+)"', text, re.M):
                if CHECKOUT_FALLBACK.get(name) == include:
                    continue
