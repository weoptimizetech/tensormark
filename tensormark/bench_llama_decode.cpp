...
int main(int argc, char** argv) {
    try {
        ...
        // Per-step token trace, for localizing where two lanes' greedy trajectories
        // part: TM_BENCH_TRACE=1 prints one TOK line per sampled token, so two runs
        // (e.g. TM_DECODE_GPU=0 vs 1) diff to the first position that disagrees.
        const bool trace = std::getenv("TM_BENCH_TRACE") != nullptr;
