...
typedef struct tm_llama_gpu_layer {
    ...
    const float* q_bias; const float* k_bias; const float* v_bias;  // Qwen2 attention bias (QD/KVD/KVD); may be null
...
[[nodiscard]] int tm_metal_llama_prefill(void* h, float* x, int T, int pos0);   // x: T x D in/out
...
[[nodiscard]] int tm_metal_llama_prefill_begin(void* h, const float* x, int T, int pos0);
[[nodiscard]] int tm_metal_llama_prefill_wait_layer(int l);
[[nodiscard]] int tm_metal_llama_prefill_end(void);
[[nodiscard]] double tm_metal_llama_prefill_gpu_ms(void);   // GPU busy time of the last prefill
