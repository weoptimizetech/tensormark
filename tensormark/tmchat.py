...
class Engine:
    ...
    def __init__(self, model: str | os.PathLike = DEFAULT_MODEL,
        ...
        # /greedy TOGGLES, and the engine's own default is greedy now (21a8683),
        # so a blind send turns it OFF into sampled replies. Settle on the STATE
        # the caller asked for rather than assuming what the command does.
    ...
    def _ensure_greedy(self, want: bool) -> None:
        ...
        /greedy is a TOGGLE: whether it enables or disables depends on how the
        engine was started, and that starting default became greedy in 21a8683 —
        which is how a blind send started serving sampled replies instead. Two
        attempts cover the whole space, since one flip separates the two states.
        """
        for _ in range(2):
