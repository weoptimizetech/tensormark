...
struct Node {
    ...
    //
    // std::function, and deliberately NOT std::move_only_function: this
    // toolchain's libc++ does not provide it. __cpp_lib_move_only_function is
    // undefined under BOTH -std=c++23 and -std=c++2c on Apple clang 17.0.0 with
    // the Xcode libc++, and that is the toolchain build.sh / tests/run_all.cpp
    // and qa.sh's core gates use, so a movable-only closure is not available
    // here. It matters beyond taste: a copyable closure cannot own a
    // unique_ptr, which is why the pooled scratch captured below is still a
    // shared_ptr. Move both together when the toolchain moves.
...
struct Tape {
    ...
    void backward(const ValueP& loss) {
        // seed: dL/dL = 1
...
// relu forward, o = x > 0 ? x : 0. NEON select of the SAME compare the scalar
// form makes, so the result (including NaN -> 0, which is what both
// std::max(0.f, x) and this select produce) is identical elementwise.
inline void tm_loop_relu(const float* __restrict x, float* __restrict o, int n) {
#if defined(TM_HAVE_NEON)
    int i = 0;
    const float32x4_t zero = vdupq_n_f32(0.f);
    for (; i + 4 <= n; i += 4) {
        float32x4_t xv = vld1q_f32(x + i);
        vst1q_f32(o + i, vbslq_f32(vcgtq_f32(xv, zero), xv, zero));
    }
    for (; i < n; ++i) o[i] = x[i] > 0.f ? x[i] : 0.f;
#else
    for (int i = 0; i < n; ++i) o[i] = x[i] > 0.f ? x[i] : 0.f;
#endif
}
// relu backward, o += (x > 0) ? g : 0 — the masked-add form of the scalar
// `if (x > 0) o[i] += g[i]`. No arithmetic beyond the add, so bit-identical.
inline void tm_loop_relu_acc(const float* __restrict x, const float* __restrict g,
                             float* __restrict o, int n) {
#if defined(TM_HAVE_NEON)
    int i = 0;
    const float32x4_t zero = vdupq_n_f32(0.f);
    for (; i + 4 <= n; i += 4) {
        ...
        float32x4_t ov = vld1q_f32(o + i);
        vst1q_f32(o + i, vaddq_f32(ov, vbslq_f32(vcgtq_f32(xv, zero), gv, zero)));
    }
    for (; i < n; ++i) if (x[i] > 0.f) o[i] += g[i];
#else
    for (int i = 0; i < n; ++i) if (x[i] > 0.f) o[i] += g[i];
#endif
}
...
inline ValueP relu(ValueP A) {
    Tensor o = Tensor::uninit(A->t->shape());
    {
        const float* a = A->t->data();
        float* od = o.data();
        tm_par(o.numel(), [=](int i0, int i1) { tm_loop_relu(a + i0, od + i0, i1 - i0); });
    }
    ...
    if (current_tape().enabled && A->requires_grad) {
        ...
        n.backward = [A](const Tensor& g) {
            ...
            const float* x = A->t->data();
                float* ad = A->grad->data();
            const float* gd = g.data();
            tm_par(A->numel(), [=](int i0, int i1) {
                tm_loop_relu_acc(x + i0, gd + i0, ad + i0, i1 - i0);
            });
        };
        v->node = current_tape().push(std::move(n));
    }
    ...
    if (current_tape().enabled && (A->requires_grad || B->requires_grad)) {
        ...
        n.backward = [A, B](const Tensor& g) {
            ...
            for (auto& p : {A, B}) {
                if (!p->requires_grad) continue;
                if (!p->grad) p->grad = std::make_shared<Tensor>(Tensor::zeros(p->shape()));
                // c = sign*s, a-b, c*(a-b), += : the scalar form's four
                // roundings in the same order. The forward sum stays a serial
                // accumulator — vDSP_sve would reorder it.
                const float c = ((p == A) ? 1.f : -1.f) * s;
                float* pd = p->grad->data();
                const float* ad = A->t->data();
                const float* bd = B->t->data();
                const int n = p->numel();
                tm_elem_stream(n, [=](int i0, int i1) {
                    auto& t = tm_scratch(std::size_t(i1 - i0), 0);
                    vDSP_vsub(bd + i0, 1, ad + i0, 1, t.data(), 1, vDSP_Length(i1 - i0));
                    vDSP_vsmul(t.data(), 1, &c, t.data(), 1, vDSP_Length(i1 - i0));
                    vDSP_vadd(pd + i0, 1, t.data(), 1, pd + i0, 1, vDSP_Length(i1 - i0));
                }, [=](int i0, int i1) {
                    for (int i = i0; i < i1; ++i)
                        pd[i] += c * (ad[i] - bd[i]);
                });
    ...
    auto stats = std::make_shared<std::vector<float>>(std::size_t(rows) * 2);  // mean, rstd
    {
        ...
        tm_par_rows(rows, cols, [=](int r0, int r1) {
            for (int r = r0; r < r1; ++r) {
                ...
                st[std::size_t(2) * r] = m; st[std::size_t(2) * r + 1] = rstd;
            ...
            if (A->requires_grad) {
                ...
                tm_par_rows(rows, cols, [=](int r0, int r1) {
                    for (int r = r0; r < r1; ++r) {
                        ...
                        const float m = st[std::size_t(2) * r], rstd = st[std::size_t(2) * r + 1];
...
inline ValueP softmax(ValueP A) {
    ...
    if (current_tape().enabled && A->requires_grad) {
        ...
        n.backward = [A, out_copy, rows, cols](const Tensor& g) {
            ...
            const float* gd = g.data();
            tm_par_rows(rows, cols, [=](int r0, int r1) {
                auto& t1 = tm_scratch(std::size_t(cols), 0);
                auto& t2 = tm_scratch(std::size_t(cols), 1);
                for (int r = r0; r < r1; ++r) {
                    const float* yr = y + std::size_t(r) * cols;
                    const float* gr = gd + std::size_t(r) * cols;
                    float* ar = ad + std::size_t(r) * cols;
                    float dot; vDSP_dotpr(gr, 1, yr, 1, &dot, vDSP_Length(cols));
                    // ar += yr * (gr - dot) in the scalar form's three
                    // roundings: subtract, multiply, add — each staged in
                    // scratch rather than one fused vma.
                    const float negdot = -dot;
                    vDSP_vsadd(gr, 1, &negdot, t1.data(), 1, vDSP_Length(cols));
                    vDSP_vmul(yr, 1, t1.data(), 1, t2.data(), 1, vDSP_Length(cols));
                    vDSP_vadd(ar, 1, t2.data(), 1, ar, 1, vDSP_Length(cols));
                }
            });
        };
        v->node = current_tape().push(std::move(n));
...
inline ValueP reshape(ValueP A, std::vector<int> shape) {
    ...
    std::memcpy(o.data(), A->t->data(), std::size_t(o.numel()) * sizeof(float));
    auto v = std::make_shared<Value>(std::move(o), true);
    if (current_tape().enabled && A->requires_grad) {
        Node n; n.inputs = {A}; n.output = v;
        n.backward = [A](const Tensor& g) {
            ...
            const float* gd = g.data();
            const int n = g.numel();
            if (fresh) { std::memcpy(ad, gd, std::size_t(n) * sizeof(float)); return; }
            tm_elem_stream(n, [=](int i0, int i1) {
                vDSP_vadd(ad + i0, 1, gd + i0, 1, ad + i0, 1, vDSP_Length(i1 - i0));
            }, [=](int i0, int i1) { tm_loop_acc(gd + i0, ad + i0, i1 - i0); });
        };
        v->node = current_tape().push(std::move(n));
    }
    return v;
...
inline ValueP concat(ValueP A, ValueP B) {
    ...
    {
        const float* a = A->t->data(); const float* b = B->t->data();
        float* o = out.data();
        tm_par_rows(rows, ca + cb, [=](int r0, int r1) {
            for (int r = r0; r < r1; ++r) {
                std::memcpy(o + std::size_t(r) * (ca + cb),
                            a + std::size_t(r) * ca, std::size_t(ca) * sizeof(float));
                std::memcpy(o + std::size_t(r) * (ca + cb) + ca,
                            b + std::size_t(r) * cb, std::size_t(cb) * sizeof(float));
            }
        });
    }
    auto v = std::make_shared<Value>(std::move(out), true);
    if (current_tape().enabled) {
        ...
        n.backward = [A, B, rows, ca, cb](const Tensor& g) {
            ...
            float* ad = A->grad->data();
            float* bd = B->grad->data();
            const float* gd = g.data();
            // Row-segment adds: the same adds the scalar form made, now a
            // contiguous vDSP_vadd per segment.
            tm_par_rows(rows, ca + cb, [=](int r0, int r1) {
                for (int r = r0; r < r1; ++r) {
                    const float* gr = gd + std::size_t(r) * (ca + cb);
                    vDSP_vadd(ad + std::size_t(r) * ca, 1, gr, 1,
                              ad + std::size_t(r) * ca, 1, vDSP_Length(ca));
                    vDSP_vadd(bd + std::size_t(r) * cb, 1, gr + ca, 1,
                              bd + std::size_t(r) * cb, 1, vDSP_Length(cb));
                }
            });
        };
        v->node = current_tape().push(std::move(n));
...
inline ValueP cross_entropy(ValueP logits, const std::vector<int>& labels,
    ...
    if (current_tape().enabled && logits->requires_grad) {
        ...
        n.backward = [logits, p, labels, B, C, valid, ignore_index](const Tensor& g) {
            ...
            tm_par_rows(B, C, [=](int r0, int r1) {
                auto& t = tm_scratch(std::size_t(C), 0);
                for (int b = r0; b < r1; ++b) {
                    if (lab[b] == ignore_index) continue;
                    const float* pr = pd + std::size_t(b) * C;
                    float* lr = ld + std::size_t(b) * C;
                    // lr += s*p in two roundings (scale, then add) — the
                    // same two the scalar form made. One fused vsma would
                    // round differently, so the product stages in scratch.
                    vDSP_vsmul(pr, 1, &s, t.data(), 1, vDSP_Length(C));
                    vDSP_vadd(lr, 1, t.data(), 1, lr, 1, vDSP_Length(C));
                    lr[lab[b]] -= s;
                }
            });
        };
        ...
        for (int n = n0; n < n1; ++n)
            for (int oy = 0; oy < oH; ++oy)
                for (int ox = 0; ox < oW; ++ox) {
                    const float* row = col + (std::size_t(n * oH + oy) * oW + ox) * K;
                    int k = 0;
                    for (int c = 0; c < C; ++c)
                        for (int ky = 0; ky < kH; ++ky)
                            for (int kx = 0; kx < kW; ++kx)
...
// The weight matrix a conv GEMM wants is (O, C*kH*kW) row-major. W is stored
// (O, C, kH, kW) row-major, so that matrix IS W's own storage: the old
// "_tm_pack_w" was an element-for-element self-copy (both sides expanded to
// o*K + c*kH*kW + ky*kW + kx) and the cache around it only ever held a second
// copy of W — plus a by-value copy per closure. The GEMMs now read
// W->t->data() directly.
//
// DEPRECATED no-op: kept only because tensormark.cpp (pybind
// `clear_weight_cache`) still calls it, and that file is outside this lane's
// ownership. Deliberately NOT marked [[deprecated]] — that would introduce a
// warning into a translation unit this lane does not own. Delete both the
// binding and this function together.
inline void tm_clear_wcache() {}
...
inline ValueP conv2d(ValueP A, ValueP W, int stride = 1) {
    ...
    if (W->shape()[2] == 1 && W->shape()[3] == 1 && stride == 1 && B == 1) {
        ...
        // W (O, C, 1, 1) row-major IS the (O, C) GEMM operand.
        const float* Wm = W->t->data();
        Tensor out = Tensor::uninit({1, O, H, Wd});  // GEMM overwrites all of it
        const float* X = A->t->data();
        float* Y = out.data();
        auto gemm_range = [&](int n0, int n1) {
            cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, O, n1 - n0, C,
                        1.0f, Wm, C, X + n0, HW, 0.0f, Y + n0, HW);
        };
    ...
    int R = B * oH * oW;
    // W (O,C,kH,kW) row-major IS the (O, C*kH*kW) GEMM operand — no repack.
    const float* Wm = W->t->data();
    // The im2col of A. Backward needs this same buffer, so it is built once here
    // (pool-backed, kept alive by the tape node) instead of rebuilt there — the
    // same forward/backward reuse neural_demo.cpp's Conv2D ships
    // (specs/TENSORMARK_M7_SCHED_SPEC.md stage 4, +11%). im2col overwrites every
    // element it owns, so the buffer needs no pre-fill.
    auto col = pooled(std::size_t(R) * K, /*zero=*/false);
    tm_im2col(A->t->data(), col->data(), B, C, H, Wd, kH, kW, oH, oW, stride);
    // psgemm emits (R, O) rows = (n,oy,ox) — NOT NCHW; permute into NCHW.
    Tensor outRO = Tensor::uninit({R, O});
    tm_gemm_nt(R, O, K, col->data(), Wm, outRO.data(), O);
    Tensor out4 = Tensor::uninit({B, O, oH, oW});
    ...
    if (current_tape().enabled && (A->requires_grad || W->requires_grad)) {
        Node n; n.inputs = {A, W}; n.output = v;
        n.backward = [A, W, col, B, C, H, Wd, O, kH, kW, oH, oW, stride, K, R, Wm](const Tensor& g) {
            (void)H; (void)kH;
            // g arrives in the OUTPUT's layout (B, O, oH, oW), but both GEMMs
            // below index their R axis as (n, oy, ox). Permute once. (This used
            // to be a flat memcpy that read the NCHW gradient as if it were
            // already (R, O) — correct only when the incoming gradient happened
            // to be constant.)
            Tensor go_mat = Tensor::uninit({R, O});
            traink::par_chunks(B * O, std::max(1, B * O / (2 * std::max(1, traink::nchunk()))),
                               [&](int p0, int p1) {
                for (int p = p0; p < p1; ++p) {
                    const int n = p / O, o = p % O;
                    for (int y = 0; y < oH; ++y)
                        for (int x = 0; x < oW; ++x)
                            go_mat[((n * oH + y) * oW + x) * O + o] =
                                g[((n * O + o) * oH + y) * oW + x];
                }
            });
            if (W->requires_grad) {
                // dW(O,K) = go_mat(R,O)^T @ col(R,K), ACCUMULATED. The old form
                // assigned into W->grad, so a weight used by two convs on one
                // tape lost every contribution but the last.
                bool fresh; float* wd = tm_grad_fresh(W, fresh).data();
                traink::psgemm_tn(O, K, R, go_mat.data(), col->data(), wd, 1.0f,
                                  fresh ? 0.0f : 1.0f);
            }
            if (A->requires_grad) {
                // col2im scatter-adds, so its target must start at zero.
                float* ad = tm_grad(A).data();
                auto dcol = pooled(std::size_t(R) * K, /*zero=*/false);
                traink::psgemm_nn_m(R, K, O, go_mat.data(), Wm, dcol->data(), 1.0f);
                tm_col2im(dcol->data(), ad, B, C, H, Wd, kH, kW, oH, oW, stride);
            }
        };
        v->node = current_tape().push(std::move(n));
    }
...
inline ValueP depthwise_conv2d(ValueP A, ValueP W, int stride = 1) {
    ...
    Tensor out = Tensor::uninit({B, C, oH, oW});  // every (n,c,oy,ox) written
            ...
            if (kH == 3 && kW == 3 && stride == 1) {
                ...
                for (int oy = 0; oy < oH; ++oy)
                    for (int ox = 0; ox < oW; ++ox) {
                        ...
                        for (int ky = 0; ky < kH; ++ky)
                            for (int kx = 0; kx < kW; ++kx)
                                acc += xc[((std::size_t)(oy * stride + ky)) * Wd + ox * stride + kx]
                                     * wc[ky * kW + kx];
                        oc[oy * oW + ox] = acc;
                    }
            }
    ...
    if (current_tape().enabled && A->requires_grad) {
        ...
        n.backward = [A, B, C, H, W, Hp, Wp, pt, pl](const Tensor& g) {
            ...
            for (int n = 0; n < B; ++n)
                for (int c = 0; c < C; ++c)
                    for (int y = 0; y < H; ++y)
                        std::memcpy(&(*A->grad)[(std::size_t(n * C + c) * H + y) * W],
                                    g.data() + (std::size_t(n * C + c) * Hp + y + pt) * Wp + pl,
                                    sizeof(float) * W);
        };
        v->node = current_tape().push(std::move(n));
    }
...
inline ValueP conv2d_ex(ValueP A, ValueP W, int stride,
    ...
    int R = B * oH * oW;
    const float* Wm = W->t->data();  // (O,C,kH,kW) IS (O, C*kH*kW) — no repack
    ...
    // Every element of col is written by im2col below.
    Tensor col = Tensor::uninit({R, K});
    tm_im2col(xp, col.data(), B, C, Hs, Ws, kH, kW, oH, oW, stride);
    Tensor out = Tensor::uninit({B, O, oH, oW});
    float* optr = out.data();
    if (B == 1) {
        // (O, oH*oW) IS NCHW for B==1: GEMM writes the output layout
        // directly, no (R,O)->NCHW permute pass.
        tm_gemm_nt(O, R, K, Wm, col.data(), optr, R);
    } else {
        Tensor outRO = Tensor::uninit({R, O});
        tm_gemm_nt(R, O, K, col.data(), Wm, outRO.data(), O);
        traink::par_chunks(B * O, std::max(1, B * O / (2 * std::max(1, traink::nchunk()))),
...
inline ValueP pad_symmetric(ValueP A) {
    int B = A->shape()[0], C = A->shape()[1], H = A->shape()[2], W = A->shape()[3];
    Tensor out = Tensor::uninit({B, C, H + 2, W + 2});  // every padded cell written
    for (int n = 0; n < B; ++n)
        for (int c = 0; c < C; ++c)
            for (int y = 0; y < H + 2; ++y) {
...
inline ValueP sigmoid(ValueP A) {
    ...
    if (current_tape().enabled && A->requires_grad) {
        Node n; n.inputs = {A}; n.output = v;
        auto out_copy = v->t;   // the forward output IS the sigmoid p
        n.backward = [A, out_copy](const Tensor& g) {
            if (!A->grad) A->grad = std::make_shared<Tensor>(Tensor::zeros(A->shape()));
            const float* pd = out_copy->data();
            float* ad = A->grad->data();
            const float* gd = g.data();
            const int n = g.numel();
            // g*p, *(1-p), += : the scalar form's three roundings in the
            // same order (a fused vDSP_vma would round differently). p comes
            // from the saved forward output — the old code recomputed
            // 1/(1+exp(-x)) per element, which produced the same value and
            // wasted an exp. The forward itself stays scalar: vvexpf is not
            // bit-identical to std::exp and the forward feeds the graph.
            tm_elem_stream(n, [=](int i0, int i1) {
                auto& t = tm_scratch(std::size_t(i1 - i0), 0);
                vDSP_vmul(gd + i0, 1, pd + i0, 1, t.data(), 1, vDSP_Length(i1 - i0));
                for (int i = 0; i < i1 - i0; ++i) t[i] *= 1.f - pd[i0 + i];
                vDSP_vadd(ad + i0, 1, t.data(), 1, ad + i0, 1, vDSP_Length(i1 - i0));
            }, [=](int i0, int i1) {
                for (int i = i0; i < i1; ++i) {
                    const float p = pd[i];
                    ad[i] += gd[i] * p * (1.f - p);
...
inline ValueP maxpool2x2(ValueP A) {
    int B = A->shape()[0], C = A->shape()[1], H = A->shape()[2], W = A->shape()[3];
    int oH = H / 2, oW = W / 2;
    Tensor out = Tensor::uninit({B, C, oH, oW});  // every window writes both
    Tensor arg = Tensor::uninit({B, C, oH, oW});
    const Tensor& At = *A->t;
    for (int n = 0; n < B; ++n)
        for (int c = 0; c < C; ++c)
            for (int y = 0; y < oH; ++y)
    ...
    if (current_tape().enabled && A->requires_grad) {
        Node n; n.inputs = {A}; n.output = v;
        // The argmax planes must outlive this call; hand them over by
        // handle. Capturing the Tensor itself COPIED the whole plane set
        // once per node.
        auto argp = std::make_shared<Tensor>(std::move(arg));
        n.backward = [A, argp, B, C, H, W, oH, oW](const Tensor& g) {
            const Tensor& arg = *argp;
            if (!A->grad) A->grad = std::make_shared<Tensor>(Tensor::zeros(A->shape()));
            for (int n = 0; n < B; ++n)
                for (int c = 0; c < C; ++c)
                    for (int y = 0; y < oH; ++y)
