...
// After pybind11: session.h's GIL-release alias needs pybind11::gil_scoped_release.
#include "session.h"
#include <cstring>
#include <memory>
#include <stdexcept>
...
PYBIND11_MODULE(tensormark, mod) {
    ...
    py::class_<Tensor, std::shared_ptr<Tensor>>(mod, "Tensor")
        ...
        // Span-style indexing: a python int indexes the FLATTENED buffer (the
        // engine's own operator[], no copy); a (i, j, ...) tuple unravels
        // row-major over the LEADING dims and spans the rest (t[1] on a (2,2)
        // tensor is row 1's span). Out-of-range raises IndexError, not the
        // engine's exception, so the python surface behaves like a list.
        .def("__getitem__", [](const Tensor& t, py::object key) -> float {
            const auto& s = t.shape();
            std::vector<py::ssize_t> idx;
            try {
                idx = key.cast<std::vector<py::ssize_t>>();
            } catch (const py::cast_error&) {
                idx = {key.cast<py::ssize_t>()};
            }
            if (idx.empty()) throw py::index_error("empty index");
            if (idx.size() > s.size()) {
                throw py::index_error("too many indices for a rank-" +
                                      std::to_string(s.size()) + " tensor");
            }
            if (idx.size() == 1 && s.size() > 1) {
                // Short index on a multi-dimensional tensor: SPAN of the first
                // dim, i.e. the flattened buffer's [i * row, (i+1) * row) — but
                // a scalar return can only hand back one element, so a span
                // index is only defined when the span has size 1; anything else
                // is an error and the caller should use .numpy() for a view.
                const std::size_t i = idx[0] < 0
                    ? std::size_t(idx[0] + py::ssize_t(s[0]))
                    : std::size_t(idx[0]);
                if (idx[0] < 0 && i >= std::size_t(s[0]))
                    throw py::index_error("index out of range for dim 0 (size " +
                                          std::to_string(s[0]) + ")");
                if (idx[0] >= s[0])
                    throw py::index_error("index " + std::to_string(idx[0]) +
                                          " out of range for dim 0 (size " +
                                          std::to_string(s[0]) + ")");
                std::size_t row = 1;
                for (std::size_t k = 1; k < s.size(); ++k) row *= std::size_t(s[k]);
                if (row != 1)
                    throw py::index_error(
                        "integer index on a rank-" + std::to_string(s.size()) +
                        " tensor spans a row of " + std::to_string(row) +
                        " elements; use .numpy() for a view or index every dim");
                return t.raw()[i];
            }
            std::size_t flat = 0;
            for (std::size_t k = 0; k < s.size(); ++k) {
                py::ssize_t i = idx[k] < 0 ? idx[k] + py::ssize_t(s[k]) : idx[k];
                if (i < 0 || i >= s[k])
                    throw py::index_error("index " + std::to_string(idx[k]) +
                                          " out of range for dim " + std::to_string(k) +
                                          " (size " + std::to_string(s[k]) + ")");
                flat = flat * std::size_t(s[k]) + std::size_t(i);
            }
            if (flat >= t.raw().size()) throw py::index_error("index out of range");
            return t.raw()[flat];
        })
...
// ---- LLM Session (finding 19): prefill/step/decode/rewind, GIL released ---
py::class_<tmsession::Session, std::shared_ptr<tmsession::Session>>(
    mod, "Session")
    .def(py::init<>([](const std::string& model, const std::string& tokenizer,
                       int ctx, unsigned seed) {
        // Construction mmaps the weights; the GIL can stay held for the
        // open() itself but holding it across a 600 MB fault-in is rude —
        // release around the whole thing.
        py::gil_scoped_release release;
        return std::make_shared<tmsession::Session>(model, tokenizer, ctx, seed);
    }),
         py::arg("model"), py::arg("tokenizer"), py::arg("ctx") = 0,
         py::arg("seed") = 1234)
    .def("prefill", &tmsession::Session::prefill, py::arg("text"),
         "Tokenize + load the cache; returns the prompt token count.")
    .def("prefill_ids", &tmsession::Session::prefill_ids, py::arg("ids"),
         "Prefill from ids instead of text.")
    .def(
        "step",
        [](tmsession::Session& s, float temperature, int top_k, float top_p,
           bool greedy, int turn_end) {
            tmsession::StepOpts o{temperature, top_k, top_p, greedy, turn_end, -1};
            const tmsession::StepResult r = s.step(o);
            py::dict d;
            d["id"] = r.id;
            d["delta"] = r.delta;
            d["done"] = r.done;
        ...
        py::arg("temperature") = 0.7f, py::arg("top_k") = 40,
        py::arg("top_p") = 0.9f, py::arg("greedy") = false,
        py::arg("turn_end") = -1,
        "Sample one token; {id, delta, done}. GIL released inside.")
    .def("decode_delta", &tmsession::Session::decode_delta, py::arg("id"),
         py::arg("first_piece"),
         "Bytes a token contributes to a running decode.")
    .def("decode", &tmsession::Session::decode,
         "Decode of every generated id so far.")
    .def("rewind", &tmsession::Session::rewind, py::arg("n"),
         "Rewind the cache to absolute token position n; refused on "
         "recurrent-layer models where a rewind is unsound.")
    .def_property_readonly("prompt_len", &tmsession::Session::prompt_len)
    .def_property_readonly("seen", &tmsession::Session::seen)
    .def_property_readonly("vocab", &tmsession::Session::vocab)
    .def_property_readonly("ctx", &tmsession::Session::ctx);
