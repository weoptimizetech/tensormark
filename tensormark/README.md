...
The build targets `apple-m1` by default — deliberately NOT the build host's CPU.
It used to be `-march=native`, which baked the build machine's instruction set
into a wheel that ships; an M3/M4 build then died with SIGILL on an M1. To target
a different CPU explicitly (e.g. a local-only native build):

```bash
TENSORMARK_ARCH=native python -m pip wheel . --no-deps -w dist
```
