...
inline void tok_gdn_conv(int in_slot, int state_slot, const void* kern,
    ...
    for (auto& e : kc)
        if (std::get<0>(e) == kern && std::get<1>(e) == kbytes) { bk = std::get<2>(e); break; }
    if (!bk) {
        bk = [dev() newBufferWithBytes:kern length:kbytes
            options:MTLResourceStorageModeShared];
        kc.push_back({kern, kbytes, bk});
    }
    [tenc() setComputePipelineState:ops_pso("gdn_conv_silu")];
    ...
    [tenc() setBuffer:const_u32(channels) offset:0 atIndex:4];
    [tenc() setBuffer:const_u32(K) offset:0 atIndex:5];
...
inline void tok_gdn_l2(int src_slot, int dst_slot, unsigned H, unsigned D,
    ...
    [tenc() setComputePipelineState:ops_pso("gdn_l2_norm")];
    [tenc() setBuffer:slot_buf(src_slot, sizeof(float) * (src_off + (size_t)H * D))
      offset:0 atIndex:0];
    [tenc() setBuffer:slot_buf(dst_slot, sizeof(float) * (dst_off + (size_t)H * D))
      offset:0 atIndex:1];
    [tenc() setBuffer:const_u32(H) offset:0 atIndex:2];
    [tenc() setBuffer:const_u32(D) offset:0 atIndex:3];
    [tenc() setBuffer:const_u32(src_off) offset:0 atIndex:4];
    [tenc() setBuffer:const_f32(scale) offset:0 atIndex:5];
    [tenc() setBuffer:const_f32(eps) offset:0 atIndex:6];
    [tenc() setBuffer:const_u32(dst_off) offset:0 atIndex:7];
...
inline void tok_gdn_head_rms(int x_slot, const void* w, int out_slot,
    ...
    [tenc() setComputePipelineState:ops_pso("gdn_head_rms")];
    ...
    [tenc() setBuffer:const_u32(D) offset:0 atIndex:4];
    [tenc() setBuffer:const_f32(eps) offset:0 atIndex:5];
    [tenc() dispatchThreadgroups:MTLSizeMake(H, 1, 1)
      threadsPerThreadgroup:MTLSizeMake(256, 1, 1)];
}
inline void tok_gdn_decay(int alpha_slot, const void* dt, const void* a,
    ...
    [tenc() setComputePipelineState:ops_pso("gdn_decay")];
    ...
    [tenc() setBuffer:const_u32(Hv) offset:0 atIndex:4];
...
inline void tok_gdn_step(int s_slot, int q_slot, int k_slot, int v_slot,
                         ...
                         unsigned o_off) {
    [tenc() setComputePipelineState:ops_pso("gdn_step")];
    [tenc() setBuffer:slot_buf(s_slot, sizeof(float) * (size_t)Hv * Dv * Dk) offset:0 atIndex:0];
    [tenc() setBuffer:slot_buf(q_slot, sizeof(float) * (q_off + (size_t)Hk * Dk)) offset:q_off atIndex:1];
    [tenc() setBuffer:slot_buf(k_slot, sizeof(float) * (k_off + (size_t)Hk * Dk)) offset:k_off atIndex:2];
    [tenc() setBuffer:slot_buf(v_slot, sizeof(float) * (v_off + (size_t)Hv * Dv)) offset:v_off atIndex:3];
    ...
    [tenc() setBuffer:slot_buf(o_slot, sizeof(float) * (o_off + (size_t)Hv * Dv)) offset:o_off atIndex:6];
    [tenc() setBuffer:const_u32(Hk) offset:0 atIndex:7];
    [tenc() setBuffer:const_u32(Hv) offset:0 atIndex:8];
    [tenc() setBuffer:const_u32(Dk) offset:0 atIndex:9];
    [tenc() setBuffer:const_u32(Dv) offset:0 atIndex:10];
...
inline void tok_gdn_silu_gate(int x_slot, int z_slot, unsigned n) {
    [tenc() setComputePipelineState:ops_pso("gdn_silu_gate")];
