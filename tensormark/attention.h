...
inline ValueP mha_causal(ValueP Q, ValueP K, ValueP V, int B, int T, int H) {
    ...
    // Every row is written by the output GEMM below (beta = 0), head by head.
    Tensor out = Tensor::uninit({B * T, D});
    ...
    if (current_tape().enabled &&
        ...
        n.backward = [Q, K, V, P, B, T, H, D, dh, sc, tile](const Tensor& g) {
            ...
            const int BH = B * H;
            const int nc = std::max(1, traink::nchunk());
            const int chunk = std::max(1, (BH + nc - 1) / nc);
            const int blocks = (BH <= chunk) ? 1 : (BH + chunk - 1) / chunk;
            // One dS panel per chunk, pooled. A thread is handed whole chunks
            // and uses its panel serially, so no two threads ever touch the
            // same panel — which is why this is one shared buffer and not a
            // per-thread slot: the body has no thread id, and one thread runs
            // several chunks. Before this, each chunk did its own fresh
            // tile*T allocation on every backward step.
            auto dS_panel = pooled(std::size_t(blocks) * tile * T, /*zero=*/false);
            float* dS = dS_panel->data();          // this chunk's panel (ld = T)
            traink::par_chunks(BH, chunk, [&, dS](int s0, int s1) {
                float* dSc = dS + std::size_t(s0 / chunk) * tile * T;
                for (int s = s0; s < s1; ++s) {
                    const int b = s / H, h = s % H;
                    const std::size_t base = std::size_t(b) * T * D + std::size_t(h) * dh;
                    const float* qb = q + base;
                    ...
                    for (int i0 = 0; i0 < T; i0 += tile) {
                        ...
                        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, rows, cols,
                                    ...
                                    0.f, dSc, T);
                        attn_detail::softmax_causal_tile_bwd(dSc, Pt, T, i0, rows, cols);
                        // dQ_tile += scale * dS @ K[0:cols]
                        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, rows, dh,
                                    cols, sc, dSc, T, kb, D, 1.f,
                                    dqb + std::size_t(i0) * D, D);
                        // dK[0:cols] += scale * dS^T @ Q_tile
                        cblas_sgemm(CblasRowMajor, CblasTrans, CblasNoTrans, cols, dh,
                                    rows, sc, dSc, T, qb + std::size_t(i0) * D, D,
                                    1.f, dkb, D);
                    }
                }
            });
