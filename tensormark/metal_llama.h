...
#include <unistd.h>
#include <atomic>
#include <compare>   // MmKey's defaulted <=>
#include <memory>
#include <map>
#include <vector>
...
[[nodiscard]] inline NSString* kSrc() {
    return @R"MET(
...
// (j, j+dh/2), position pos0 + row — the CPU rope_head convention. The angle
// is the host-built (cos, sin) table, the SAME one the decode rope kernels
// read: computing it here with precise::pow/cos/sin instead made a prefill K
// row and a decode K row for one position differ in the last bits.
kernel void ll_rope(device float* q [[buffer(0)]], constant uint& nheads [[buffer(1)]],
    ...
    if ((size_t)(pos0 + t) * half_ + j >= tbl_n) return;
    const float2 cs = tbl[(size_t)(pos0 + t) * half_ + j];
    device float* r = q + (size_t)t * nheads * dh + h * dh;
    const float a = r[j], b = r[j + half_];
    r[j] = a * cs.x - b * cs.y;
    r[j + half_] = b * cs.x + a * cs.y;
}
...
kernel void ll_rope_h(device half* q [[buffer(0)]], constant uint& nheads [[buffer(1)]],
    ...
    r[j] = (half)(a * cs.x - b * cs.y);
    r[j + half_] = (half)(b * cs.x + a * cs.y);
}
...
// Row-bias add for Qwen2 attention: y[gid] += bias[gid % N] over a T x N
// matrix (T rows, N columns). One thread per element.
kernel void ll_bias(device float* y [[buffer(0)]],
...
struct Layer {
    ...
    id<MTLBuffer> q_bias, k_bias, v_bias;   // Qwen2 attention bias (copied); nil when absent
};
struct Ctx {
    ...
    // Cache key for the attention GEMM objects. A struct with a defaulted
    // three-way comparison replaces the snprintf'd string key attn_mm used: no
    // formatting, no 128-byte buffer, no locale-dependent conversion of the
    // floats, and the integer fields stay integers when they are compared.
    // `alpha` is part of the key because it is a semantic parameter of
    // MPSMatrixMultiplication - the old key left it out, which happened to be
    // harmless only because the two kinds that existed already disagreed on it.
    struct MmKey {
        enum class Kind : uint8_t { qk, pv };
        Kind kind;
        unsigned M, N, K, ldA, ldB, ldC;
        bool transB, half;
        float alpha;
        auto operator<=>(const MmKey&) const = default;
    };
    std::map<MmKey, Mm> mm;              // attention GEMM objects by shape
};

[[nodiscard]] inline id<MTLComputePipelineState> pso(Ctx& c, const char* name) {
...
[[nodiscard]] inline bool ensure_scratch(Ctx& c, int T) {
...
[[nodiscard]] inline Ctx::Mm& attn_mm(Ctx& c, Ctx::MmKey::Kind kind, unsigned M, unsigned N, unsigned K, unsigned ldA, unsigned ldB, unsigned ldC, bool transB, float alpha, bool half = false) {
    const Ctx::MmKey key{kind, M, N, K, ldA, ldB, ldC, transB, half, alpha};
...
[[nodiscard]] inline bool encode_layer(Ctx& c, id<MTLCommandBuffer> cb, int l, int T, int pos0, bool hmode, int Mp) {
    Layer& L = c.layers[l];
    ...
    // 1b. Qwen2 attention bias (per-column add, fp32 regardless of hmode
    // since Q/Kb/Vb are fp32 at this point); nil for Llama-2.
    if (L.q_bias) {
        id<MTLComputeCommandEncoder> e = enc();
        [e setComputePipelineState:pso(c, "ll_bias")];
        [e setBuffer:Q offset:0 atIndex:0]; [e setBuffer:L.q_bias offset:0 atIndex:1]; set_u32(e, QD, 2);
        elem(e, (unsigned)T * QD);
        [e setBuffer:Kb offset:0 atIndex:0]; [e setBuffer:L.k_bias offset:0 atIndex:1]; set_u32(e, KVD, 2);
        elem(e, (unsigned)T * KVD);
        [e setBuffer:Vb offset:0 atIndex:0]; [e setBuffer:L.v_bias offset:0 atIndex:1];
        elem(e, (unsigned)T * KVD);
        [e endEncoding];
    }
    // 2. rope + kv append (+ half cache copies in hmode)
    {
        // The angle table is shared with the decode lane's rope kernels; the
        // rows this layer rotates must already be in it.
        if (!::tmgpu::ensure_rope_table(dh, (unsigned)(pos0 + T), c.theta)) return false;
        auto& rt = ::tmgpu::rope_table();
        id<MTLComputeCommandEncoder> e = enc();
        ...
        [e setBuffer:rt.buf offset:0 atIndex:4];
        set_u32(e, ::tmgpu::rope_table_entries(dh), 5);
    ...
    for (int g = 0; g < ((diag == 1 || flash) ? 0 : c.KVH); ++g) {
        for (int t0 = 0; t0 < T; t0 += B) {
            ...
            elem(e, rows * dh);
            [e endEncoding];
            Ctx::Mm& m1 = attn_mm(c, Ctx::MmKey::Kind::qk, rows, allow_max, dh, dh, dh, ld, true, scale, hmode);
            ...
            [e endEncoding];
            Ctx::Mm& m2 = attn_mm(c, Ctx::MmKey::Kind::pv, rows, dh, allow_max, ld, dh, dh, false, 1.0f, hmode);
...
inline void* create(const tm_llama_gpu_desc* d) {
    ...
    @autoreleasepool {
        ...
        NSError* err = nil;
        c->lib = [detail::dev() newLibraryWithSource:kSrc()
                                             options:detail::compile_options() error:&err];
        if (!c->lib) {
            std::fprintf(stderr, "[metal-llama] library compile failed: %s\n", err.localizedDescription.UTF8String);
            return nullptr;
        }
        for (const char* k : {"ll_rmsnorm", "ll_rope", "ll_kv_append", "ll_gather_q", "ll_scatter_o", "ll_softmax", "ll_silu_mul", "ll_add", "ll_bias",
        ...
        for (int l = 0; l < c->L; ++l) {
            ...
            if (s.q_bias && s.k_bias && s.v_bias) {
                L.q_bias = [detail::dev() newBufferWithBytes:s.q_bias length:4 * (size_t)c->QD options:MTLResourceStorageModeShared];
                L.k_bias = [detail::dev() newBufferWithBytes:s.k_bias length:4 * (size_t)c->KVD options:MTLResourceStorageModeShared];
                L.v_bias = [detail::dev() newBufferWithBytes:s.v_bias length:4 * (size_t)c->KVD options:MTLResourceStorageModeShared];
            }
...
inline void destroy(void* h) { delete static_cast<Ctx*>(h); }
    ...
    //
    // Completion protocol: begin() encodes encodeSignalEvent on every layer's
    // command buffer with monotonically rising values, and wait_layer(l)
    // blocks in the kernel on [host event waitValue:base + l + 1] — the wait
    // is scheduled on the GPU, so the hybrid caller's CPU rows run without a
    // 50 µs usleep poll burning a core per layer, and the per-layer flags
    // array (plus its retired-arrays list for handlers that outlive a timed
    // out wait) is gone: MTLSharedEvent is refcounted by the command buffers
    // themselves, so there is no host object whose lifetime must be stretched
    // past an in-flight buffer. Values rise across prefills because a wait
    // returns as soon as the value is >= the target — reusing a value would
    // make the next prefill's wait_layer return before its layer finished.
    // A failed layer is signalled too (wait_layer sees a delta < 1 from the
        // completion count and reports failure through the deadline path).
        struct PrefillEvent {
        __strong id<MTLSharedEvent> ev;
        uint64_t base = 0;              // value of the LAST completed prefill
        size_t n = 0;                   // layers signalled in the live prefill
        bool failed = false;
        };
        [[nodiscard]] inline PrefillEvent& prefill_event() { static PrefillEvent e; return e; }
    [[nodiscard]] inline std::atomic<double>& prefill_gpu_busy_ms() {
        static std::atomic<double> v{0.0}; return v;
    }
    // Span of the GPU's work for the live prefill: armed by prefill_begin()
    // (every layer is committed before it returns, so that instant is the GPU's
    // start) and closed ONCE by the last layer's probe. hybrid_update() balances
    // this against cpu_ms. It used to be accumulated from per-layer GPUEndTime
    // and was deleted as "dead work plus an unused-variable warning" — but the
    // controller reads it, and while it was permanently 0 update() returned
    // immediately, so the learned fraction could never adapt (2026-09-17). The
    // `open` flag is load-bearing: prefill_end() re-probes every layer, so an
    // unguarded arm would clock the span from that replay and report ~1 ms.
    [[nodiscard]] inline std::chrono::steady_clock::time_point& prefill_span0() {
        static std::chrono::steady_clock::time_point t;
        return t;
    }
    [[nodiscard]] inline bool& prefill_span_open() {
        static bool b = false;
        return b;
    }
    [[nodiscard]] inline bool prefill_begin(void* h, const float* x, int T, int pos0);
    // The per-layer completion probes get their OWN queue. A command buffer
    // committed to the SAME queue as the prefill's layer buffers runs in commit
    // order, i.e. BEHIND them, so the probe for layer 0 could not even START
    // until the whole prefill had — measured 2026-09-17: wait(l=0) = 1931 ms
    // while every later probe was 0.1 ms, and the hybrid then ran its CPU rows
    // strictly AFTER the GPU instead of one layer behind it (wall 3773 ms ~=
    // cpu_ms 1898 + gpu 1875). The overlap is the entire point of the split.
    [[nodiscard]] inline __strong id<MTLCommandQueue>& probe_q() {
        static __strong id<MTLCommandQueue> wq = nil;
        if (wq == nil) wq = [detail::dev() newCommandQueue];
        return wq;
    }
    [[nodiscard]] inline bool prefill_wait_layer(int l) {
    auto& e = prefill_event();
    if (l < 0 || (size_t)l >= e.n) return false;
    if (e.failed) return false;
    const uint64_t target = e.base + (uint64_t)l + 1;
    // MTLSharedEvent has no polling getter (waitUntilValue:timeout: blocks the
    // caller's thread indefinitely), so the bound comes from an encoded host
    // wait: a tiny command buffer whose only work is encodeWaitForEvent, with
    // the deadline enforced on the host. This does not spin on a flags array
    // (the CPU-side cost of the old poll) and it keeps the ordering proof end
    // to end: the wait completes only after layer l's signal landed on the
    // shared event.
    @autoreleasepool {
        const auto wt0 = std::chrono::steady_clock::now();
        id<MTLCommandBuffer> wcb = [probe_q() commandBuffer];
        [wcb encodeWaitForEvent:e.ev value:target];
        [wcb commit];
        const auto deadline =
            std::chrono::steady_clock::now() + std::chrono::seconds(30);
        // Wait on EVERY pre-completion status, not just Scheduled: after commit
        // the buffer sits in Committed (2) until the scheduler moves it to
        // Scheduled (3), and waiting only on Scheduled fell through that window
        // and reported failure for a layer the GPU had not even started — the
        // hybrid case flapped between runs.
        while (wcb.status != MTLCommandBufferStatusCompleted &&
               wcb.status != MTLCommandBufferStatusError &&
               std::chrono::steady_clock::now() < deadline)
            usleep(50);
        if (std::getenv("TM_LLAMA_HYBRID_DEBUG"))
            std::fprintf(stderr, "[wait] l=%2d target=%llu waited=%7.1f ms status=%d\n", l,
                         (unsigned long long)target,
                         std::chrono::duration<double, std::milli>(
                             std::chrono::steady_clock::now() - wt0).count(),
                         (int)wcb.status);
        if (wcb.status != MTLCommandBufferStatusCompleted) {
            e.failed = true;
            return false;
        }
    }
    // Close the GPU busy span exactly once, on the last layer's probe.
    if (prefill_span_open() && (size_t)l + 1 == e.n) {
        prefill_gpu_busy_ms().store(std::chrono::duration<double, std::milli>(
            std::chrono::steady_clock::now() - prefill_span0()).count());
        prefill_span_open() = false;
    }
    return true;
}
    [[nodiscard]] inline double& prefill_gpu_ms() { static double v = 0; return v; }
    [[nodiscard]] inline bool prefill_end() {
        auto& e = prefill_event();
        bool ok = true;
        for (size_t l = 0; l < e.n; ++l)
            if (!prefill_wait_layer((int)l)) ok = false;
        prefill_gpu_ms() = prefill_gpu_busy_ms().exchange(0.0);  // GPU busy ms, for the split controller
        if (ok) e.base += (uint64_t)e.n;   // next prefill's values start above
        e.n = 0;
        return ok;
    }
    [[nodiscard]] inline bool prefill(void* h, float* x, int T, int pos0) {
        ...
        @autoreleasepool {
            ...
            // fp16 glue is only validated for dh=64 (TinyLlama); Qwen's dh=128
            // overflows fp16 in the attention path, so keep fp32 glue there
            // (correctness over the small prefill speed gain).
            const bool hmode = half_ok && c->dh == 64;
    ...
    [[nodiscard]] inline bool prefill_begin(void* h, const float* x, int T, int pos0) {
        ...
        @autoreleasepool {
            ...
            // (correctness over the small prefill speed gain).
            const bool hmode = half_ok && c->dh == 64;
            // A failed prefill leaves its event base unadvanced; the next
            // begin() re-signals the same values, which wait_layer treats as
            // "already done" only if they really did complete.
            auto& ev = prefill_event();
            if (ev.n) (void)prefill_end();   // drain best-effort
            if (!ev.ev) {
                ev.ev = [detail::dev() newSharedEvent];
            ...
            // No per-layer host-encode timing here: the hybrid lane's only reader
            // of those numbers is prefill()'s verbose report, which runs on the
            // non-hybrid path. Collecting (and never reading) them was dead work
            // plus an unused-variable warning.
            for (int l = 0; l < c->L; ++l) {
                id<MTLCommandBuffer> cb = [detail::q() commandBuffer];
                if (!encode_layer(*c, cb, l, T, pos0, hmode, Mp)) return false;
                [cb encodeSignalEvent:ev.ev value:ev.base + (uint64_t)l + 1];
                [cb commit];
            }
            // Arm the GPU busy span here: every layer is committed, so this
            // instant is the GPU's start for this prefill.
            prefill_span0() = std::chrono::steady_clock::now();
            prefill_span_open() = true;
            ++prefill_count();
        }
        return true;
    }
