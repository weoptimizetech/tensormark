...
extern "C" [[nodiscard]] bool tm_metal_tok_begin(void) { return tmgpu::tok_begin(); }
extern "C" [[nodiscard]] bool tm_metal_tok_flush(void) { return tmgpu::tok_flush(); }
extern "C" [[nodiscard]] bool tm_metal_tok_reserve(int slot, size_t bytes) {
    return tmgpu::tok_reserve(slot, bytes);
}
extern "C" [[nodiscard]] bool tm_metal_tok_end_async(void) { return tmgpu::tok_end_async(); }
extern "C" [[nodiscard]] bool tm_metal_tok_wait_pending(void) { return tmgpu::tok_wait_pending(); }
...
extern "C" [[nodiscard]] unsigned tm_metal_tok_peek(int tokslot, unsigned idx) { return tmgpu::tok_peek(tokslot, idx); }
...
extern "C" bool tm_metal_tok_gemv_seg_supported(unsigned dtype) {
    return tmgpu::tok_gemv_seg_supported(dtype);
}
extern "C" void tm_metal_tok_gemv_seg_dt(int xslot, int wid, int yslot,
    ...
    tmgpu::tok_gemv_seg_dt(xslot, wid, yslot, yoff, N, K, dtype, seg_woff, seg_rows, nseg);
}
...
// K-quant weights (Q4_1/Q5_K/Q6_K): the same token-lane contract as
// tm_metal_tok_gemv, with the dtype carried because the three kernels differ only
// in how a block is unpacked.
extern "C" void tm_metal_tok_gemv_kq(int xslot, int wid, uint64_t woff,
                                     int yslot, size_t yoff, unsigned N,
                                     unsigned K, unsigned dtype) {
    tmgpu::tok_gemv_kq(xslot, wid, woff, yslot, yoff, N, K, dtype);
}
extern "C" void tm_metal_tok_embed_kq(int tokslot, unsigned idx, int wid,
                                      uint64_t woff, int xslot, unsigned D,
                                      unsigned dtype) {
    tmgpu::tok_embed_kq(tokslot, idx, wid, woff, xslot, D, dtype);
}
...
extern "C" void tm_metal_tok_bias_qkv(int yslot, int biasslot, size_t bias_off,
                                      unsigned N) {
    tmgpu::tok_bias_qkv(yslot, biasslot, bias_off, N);
}
...
extern "C" [[nodiscard]] int tm_metal_llama_prefill(void* h, float* x, int T, int pos0) { return tmgpu::llama_gpu::prefill(h, x, T, pos0) ? 1 : 0; }
extern "C" [[nodiscard]] int tm_metal_llama_prefill_begin(void* h, const float* x, int T, int pos0) { return tmgpu::llama_gpu::prefill_begin(h, x, T, pos0) ? 1 : 0; }
extern "C" [[nodiscard]] int tm_metal_llama_prefill_wait_layer(int l) { return tmgpu::llama_gpu::prefill_wait_layer(l) ? 1 : 0; }
extern "C" [[nodiscard]] int tm_metal_llama_prefill_end(void) { return tmgpu::llama_gpu::prefill_end() ? 1 : 0; }
extern "C" [[nodiscard]] double tm_metal_llama_prefill_gpu_ms(void) { return tmgpu::llama_gpu::prefill_gpu_ms(); }
