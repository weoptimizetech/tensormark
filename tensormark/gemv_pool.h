...
// The kernel's view of one dispatch. Spans are sized by the DISPATCH site —
// the only place that knows the batch width T — so a kernel can never read a
// pointer whose extent it has to guess. `w` stays opaque: grouped/batched
// kernels pack their own layout behind it. Serial callers and the pool's
// run_slices hand the SAME args to the kernel, differing only in o0/o1.
struct GemvArgs {
    const void* w = nullptr;
    std::span<const float> x{};
    std::span<const std::int8_t> xq{};   // element-order int8 x (Q8 sdot)
    std::span<const float> xs{};         // per-32-block x scales
    std::span<const std::int8_t> xd{};   // deinterleaved (even|odd) x (Q4 sdot)
    std::span<float> y{};                // BASE of the output (kernels index
                                         // global rows; see run_slices NOTE)
    int n_in = 0;
};

// The row-claim size a dispatch gets when the call site has no opinion: 64.
// COMPILE-TIME on purpose — the runtime-adaptive variant (n_out/(2*nthreads),
// 254226b) was reverted in 8652581 because a claim that grows with the tensor
// degenerates into a static split (~8 claims over 4 threads on a 4096-row
// tensor) and loses 12-15 % the moment the OS deschedules a worker.
//
// 64 vs 128 is a measured two-regime trade, and only a fully-idle box prefers
// the coarser one: 128 wins by 1.5 % at 96 % idle, while 64 wins by 3.4 % at one
// busy P-core and 2.1-2.5 % at two (EVERY leg, not just the mean) and ~8 %
// saturated — the crossover sits between 0 % and 25 % load. 64 is the default
// because a box running a browser is never the 96 %-idle case, so the idle cost
// is the price of being faster whenever it is not. It stays overridable so the
// sweep that settled it (-DTM_GEMV_CHUNK=64|128|256) is repeatable.
#ifndef TM_GEMV_CHUNK
#define TM_GEMV_CHUNK 64
#endif

struct GemvJob {
    void (*kern)(const GemvArgs&, int o0, int o1) = nullptr;
    GemvArgs a{};
    int n_out = 0, chunk = TM_GEMV_CHUNK;
};

struct GemvPool {
    GemvJob job{};
    // One cache line each. In a single dispatch the producer bumps `next`
    // once per chunk while every worker bumps `done` and spins on `gen`, so
    // packing all three into one line makes each `done.fetch_add` invalidate
    // the line the producer is fetching `next` from — and the spin loop
    // re-reads `gen` on that same line, so the false sharing lands exactly on
    // the waiters. GemvPool is a heap singleton (`gemv_pool()`), so the
    // padding costs nothing structural.
    alignas(std::hardware_destructive_interference_size) std::atomic<std::uint64_t> next{0};
    alignas(std::hardware_destructive_interference_size) std::atomic<int> done{0};
    alignas(std::hardware_destructive_interference_size) std::atomic<std::uint64_t> gen{0};
    std::mutex mu;
    ...
    explicit GemvPool(int requested_threads = 0,
        ...
        // Re-checked 2026-09-13 on a Gated-DeltaNet model (qwen3.5 0.8B), where
        // the window DOES expire between GEMVs: its layers put a conv, two L2
        // norms, the delta-rule step and a gated RMS norm between consecutive
        // dispatches. Raising the cap to 1048576 measured 0.834x with the
        // default winning 6/6 interleaved rounds — spinning longer is not a
        // lever, because the workers burn the P-core issue slots the GEMV
        // itself needs. The ~6 ms/token of per-dispatch cost on that model is
        // therefore barrier/tail latency on cold streams, not wake latency.
    ...
    void dispatch(const GemvJob& j) {
        if (!j.kern || j.chunk <= 0 || j.a.n_in < 0 || j.n_out < 0)
