...
public:
    ...
    // Hash so the closure's visited set is a lookup, not a scan: an epsilon
    // cycle (`root ::= root*`) generates a config per return-stack depth, and a
    // linear `std::find` over full vectors makes that O(N^2 * depth).
    struct ConfigHash {
        std::size_t operator()(const Config& c) const {
            std::size_t h = static_cast<std::size_t>(c.pc) * 0x9e3779b97f4a7c15ull;
            for (int r : c.ret) h = (h ^ static_cast<std::size_t>(r)) * 0x100000001b3ull;
            return h;
        }
    };
    ...
    // Hash of a State: pc/ret pairs in their canonical (sorted, deduplicated)
    // order. The mask's allowed-token cache used to key on State::key(), a
    // serialized string with a std::to_string per integer — one string build per
    // lookup, on every decode step while a grammar is loaded.
    struct StateHash {
        [[nodiscard]] std::size_t operator()(const State& s) const noexcept {
            std::size_t h = std::hash<std::string_view>{}(s.pending);
            std::size_t hc = 0;
            for (const Config& c : s.cfg) {
                hc = hc * 0x9E3779B97F4A7C15ULL ^ static_cast<std::size_t>(c.pc);
                for (int r : c.ret) hc = hc * 0x9E3779B97F4A7C15ULL ^ static_cast<std::size_t>(r + 1);
                hc ^= hc >> 29;
            }
            return h ^ (hc + 0x9E3779B97F4A7C15ULL + (h << 6) + (h >> 2));
        }
    };
    ...
    // A dropped result means the caller believed a byte was accepted when the
    // grammar rejected it, which is not a state to walk on from.
    [[nodiscard]] bool advance(State& s, unsigned char byte) const {
...
private:
    ...
    void closure(State& s) const {
        ...
        // Epsilon closure: expand Split/Jump/Call/Return until only Char and
    // Match remain. The visited set is also what terminates nullable
    // repetition loops, where the split can reach itself without consuming
    // input.
    //
    // TWO bounds, because a cycle can blow up either dimension:
    //
    //  * DEPTH (max_calls). Real recursion deepens its return stack only as it
    //    CONSUMES input — across closures — so the stack a closure is handed
    //    can already be deep and must not be clipped. Inside one epsilon
    //    closure no character is consumed, so a cycle-free grammar can enter
    //    only as many distinct rules as it has; the never-consuming cycle
    //    (`root ::= root*`) has no such ceiling and is cut here. The bound is
    //    relative to the incoming depth, not absolute, or it would clip a
    //    legitimately deep stack handed over by an earlier closure.
    //
    //  * COUNT (kMaxConfigs). With several mutually-recursive rules the set of
    //    distinct return stacks multiplies — stacks like [a,r,b,r,...] make
    //    ~2^depth of them — so a depth bound alone still leaves an exponential
    //    NUMBER of configs, each copying a vector: libFuzzer's corpus found
    //    such a grammar and froze `start()` for minutes. A real closure
    //    enumerates hundreds of configs, never this many (deep recursion is
    //    deep in STACK, not in count), so the cap is slack for every grammar
    //    that is not an epsilon cycle. Hitting it stops expansion, which
    //    under-approximates the language of a cyclic grammar — the same
    //    fail-closed direction as rejecting malformed input, and a grammar
    //    that never terminates is not one a model can sample from anyway.
    constexpr std::size_t kMaxConfigs = 1u << 16;
    std::size_t base_depth = 0;
    for (const Config& c : s.cfg)
        base_depth = std::max(base_depth, c.ret.size());
    const std::size_t max_calls = base_depth + 2 * rules_.size() + 8;
    while (!work.empty() && seen.size() < kMaxConfigs) {
            Config c = work.back();
            work.pop_back();
            ...
            switch (in.op) {
                ...
                case Op::Split:
                    ...
                    break;
                case Op::Call: {
                    if (c.ret.size() >= max_calls) break;   // unbounded epsilon cycle
...
public:
    ...
    // The ids allowed in this state, in ascending order. Memoized per state,
    // keyed on the state itself — see Grammar::StateHash for why the old
    // serialize-to-string key is gone.
    const std::vector<int>& allowed(const Grammar::State& s) const {
        ...
        std::sort(out.begin(), out.end());
        out.erase(std::unique(out.begin(), out.end()), out.end());
        return cache_.emplace(s, std::move(out)).first->second;
    }

    // Advance a confirmed token: the state after consuming exactly its bytes.
    // Returns false if the token was not actually allowed.
    [[nodiscard]] bool consume(Grammar::State& s, int id, const std::vector<std::string>& token_text) const {
...
private:
    ...
    int eos_ = -1;
    // Keyed on the state itself (see Grammar::StateHash); clear_cache() drops it
    // when a caller rewinds a state out of band.
    mutable std::unordered_map<Grammar::State, std::vector<int>, Grammar::StateHash> cache_;

public:
    // Drops the memoized allowed-token sets. The cache is keyed on the full
    // state, so it is always CONSISTENT — this exists for memory: a long chat
    // that switches grammars per request would otherwise grow one entry per
    // distinct decode state forever. The GrammarFilter reset path calls it.
