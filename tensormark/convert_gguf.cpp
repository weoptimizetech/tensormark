...
//
// These live in gguf.h so fuzz/fuzz_gguf.cpp drives the SAME code that ships
// here rather than a copy that can drift. They return false on a malformed
// value (unknown type, over-deep nesting, absurd element count) instead of
// exiting, so the fuzzer can treat a refusal as data; this caller turns a false
// into the same hard error it always produced.
using tmg::gguf::rd_u64;
using tmg::gguf::rd_u32;
using tmg::gguf::rd_str;
using tmg::gguf::skip_value;
using tmg::gguf::skip_elems;
...
// Unaligned little-endian scalar load. A block's fp16 scale sits at whatever
// byte offset the file layout produced (2 for Q4_0, 208 for Q6_K, ...), so a
// `*reinterpret_cast<const std::uint16_t*>` there is both an unaligned access
// and a strict-aliasing violation. memcpy lowers to a single load.
template <class T>
...
void dequant_blocks(std::uint32_t type, const std::uint8_t* raw,
                    [[maybe_unused]] std::uint64_t b0,   // caller already offset `raw`
    ...
    for (std::uint64_t b = 0; b < c; ++b) {
        ...
        if (type == GGUF_Q4_0 || type == GGUF_Q4_1) {
            const float d = fp16_to_fp32(load_le<std::uint16_t>(src));
            const std::uint8_t* qs = src + (type == GGUF_Q4_1 ? 4u : 2u);
            const float m = type == GGUF_Q4_1
                ? fp16_to_fp32(load_le<std::uint16_t>(src + 2)) : 0.f;
            ...
            const float d = fp16_to_fp32(load_le<std::uint16_t>(src));
            const float mn = fp16_to_fp32(load_le<std::uint16_t>(src + 2));
            ...
            const float d = fp16_to_fp32(load_le<std::uint16_t>(src + 208));
            ...
            if (type == GGUF_F32) std::memcpy(out, src, 4);
                ...
                out[0] = fp16_to_fp32(load_le<std::uint16_t>(src));
            else out[0] = bf16_to_float(load_le<std::uint16_t>(src));
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    for (std::uint64_t i = 0; i < kv_count; ++i) {
        ...
        if (t != 9) { if (!skip_value(f, t)) fail("bad metadata value (type " + std::to_string(t) + ")"); continue; }
        ...
        else if (!skip_elems(f, et, n)) fail("bad metadata array (element type " + std::to_string(et) + ")");
    }
    // rd_str fails the stream rather than allocating when a length is absurd;
    // catch that here so a malformed file is reported, not silently read as
    // empty strings.
    if (!f) fail("truncated or malformed gguf metadata");
