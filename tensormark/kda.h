...
#pragma once
#include "autograd.h"
#include <cstring>   // std::memcpy (ut_transform's pre-update row snapshot)
...
inline void ut_transform(const float* k, const float* beta, int cn, int ldt,
    ...
    // The snapshot is one reused buffer, not a fresh vector per row (this was
    // cn allocations per call, each ~cn floats). `row` must stay a copy: the
    // j-loop reads entries this loop has not yet written and entries it has, so
    // reading Ti0 live would fold updated values into later sums.
    std::vector<float> row((std::size_t)cn);
    for (int i = 1; i < cn; ++i) {
        float* Ti0 = Tm + std::size_t(i) * ldt;
        std::memcpy(row.data(), Ti0, sizeof(float) * (std::size_t)cn);
        for (int j = 0; j < i; ++j) {
            float acc = 0.f;
...
inline ValueP kda_causal(ValueP Q, ValueP K, ValueP V, ValueP G, ValueP Beta,
    ...
    if (current_tape().enabled &&
        ...
        n.output = y;
        n.backward = [Q, K, V, G, Beta, T, H, D, dh, BH, PP](const Tensor& gt) {
