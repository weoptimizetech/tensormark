...
// The persistent worker drives the in-engine ANE dispatch API (ane_ready /
// set_ane_enabled / ane_prefills) that was removed from llama.h when the lane
// concluded the ANE cannot add to the hybrid, so it does not compile against the
// current headers (docs/ANE.md, "Where this stops"). It lived in tensormark/
// until it was moved here, and tools/bench_ane_capacity.py still names its old
// build path. Rebuilding it means restoring that API.
//
