// Status: RETIRED — kept as the record of the ANE lane; no script builds this.
// It drives the in-engine ANE dispatch API (ane_ready / set_ane_enabled /
// ane_prefills) that was removed from llama.h when the lane concluded the ANE
// cannot add to the hybrid, so it does not compile against the current headers
// (docs/ANE.md, "Where this stops"). It lived in tensormark/ until it was moved
// here. Rebuilding it means restoring that API, or rewriting it against
// tensormark/ane.h — the shim's C API, which did survive.
//
// The TM_ANE_APPROX_KV opt-in mentioned below belonged to the hidden-only
// package flavour; that flavour is gone from the packer, the shim and ane.h.
//
