# ANE prefill: the measurement record (lane retired from the engine)

**Status: retired.** The ANE prefix was built, measured — this document is that
record — and then removed from the engine. The in-engine dispatch entry points
(`ane_ready`, `set_ane_enabled`, `ane_prefills`) were taken out of `llama.h` once
coexistence showed the ANE cannot add to the hybrid, and no shipping binary reads
`TM_ANE=1` today: the two benchmarks that produced these numbers no longer compile
against the current engine, so they live in `docs/retired/` as the record instead
of in the source directory. The packer (`tools/anepack.py`), the shim
(`tensormark/ane_shim.mm` + `ane.h`) and the measurements below survive; the
engine's prefill path is the GPU/CPU hybrid. For where the lane stopped and why,
see "Where this stops" just below.

As built, the lane ran a fixed-shape prefill prefix on CoreML configured for CPU +
ANE, exported its real K/V cache, and finished the remaining layers with the Metal
layer stack. It was fp16 inference, not bit-exact CPU inference, and it did not
implement a three-unit serving controller or establish aggregate capacity under
concurrent load.

## Where this stops

The prefix depth is not a tuning knob with headroom. On this M1 the largest
single CoreML program that will build is **16 layers**, and the axis that bounds
it is **op count, not graph size**: the 16-layer program holds 1942 ops and
runs; the 22-layer one holds 2668 and is refused at load (error -14, "failed to
build model execution plan") — even though the 16-layer program carries the
*larger* weight file, 1412 MB against 855 MB. Bytes are not the constraint; the
execution-plan table is.

So the shape of the feature is fixed: ANE takes a prefix, Metal finishes the
model. Within that shape one arrangement is fast — the **fused multi-layer
graph**, which is what this guide builds and where the numbers below come from.
Dispatching one kernel per function through public CoreML is not: each dispatch
costs 551-950 ms against roughly 28 ms of compute, about 66 tok/s, and the tens
of microseconds that would make such orchestration worthwhile are not reachable
from public API.

Expect this picture to move only with different silicon — a part that can hold
the whole program at once — or a supported fast-dispatch path. Not with deeper
tuning on this one.
...
That is the half of the flow which still runs: the packer builds the fused segment.
Handing the compiled segment to the engine needed the dispatch API that has since
been removed, so the benchmark command that used to follow cannot be built any more
and is not reproduced here (its source is `docs/retired/bench_ane_split.cpp`; it
compiles again only if that API is restored). The paragraph below describes that
retired benchmark, because the numbers in this document have to be readable
against the CLI that produced them.

The positional arguments were model, token count, rounds, optional
...
The builder exports actual per-layer K/V, and that is now the only shape it
builds: the hidden-only `--no-kv` flavour is gone from the packer, from the shim
and from `ane.h`. It never could serve a normal continuation — reconstructing
every earlier cache from the final hidden is mathematically different — and its
only guard (`TM_ANE_APPROX_KV`) had been removed from the engine, so the mode had
been unreachable since the dispatch API went. `tm_ane_open` now refuses any
package whose outputs are not `h` plus `k_<i>`/`v_<i>` for every segment layer.
The earlier claim that K/V export fails at T=1024 was caused by a different,
broken float32 mask graph; corrected N16/T1024 export works.
...
historical measurements remain recorded. That key is the identity of an
already-built package: the packer hashes its own source into the key, so the
current builder does not reproduce it, and a re-run writes a new key rather than
reusing that directory. The archive and the compiled package above are the
artifacts to recover; neither needs the key to be reproducible.
...
```sh
# Nothing builds bench_prefill_lane any more (docs/retired/bench_prefill_lane.cpp
# needs the removed in-engine ANE dispatch API); this probe ran against the
# binary that build_llama.sh used to produce, with the existing compiled package.
...
# torch (the packer's graph and cache identity, coremltools mocked):
python -m unittest discover -s tools/tests -p test_anepack.py -v
# CoreML/torch environment (builds the shim and tiny generated fixtures):
python -m unittest discover -s tools/tests -p test_ane_runtime.py -v
```

The Python gates are the ANE regression surface, and they ship. The first two run
anywhere — compressed-weight residency, and the retired benchmark's input/rejection
contracts, which skip themselves because nothing builds that binary any more (the
skip message names the source and the build path that would re-enable them).
`test_anepack.py` needs torch and mocks coremltools. `test_ane_runtime.py` is the
shim's own gate: it compiles `tensormark/tests/test_ane_shim.cpp` against
`tensormark/ane_shim.mm` and runs it over generated packages, so it needs a
CoreML-capable torch (coremltools); without one it cannot run at all.

Two C++ gates used to be listed here, `build/test_ane_dispatch` and
`build/test_llama_gpu_suffix`, and they are gone: both pinned an in-engine ANE
dispatch API that was since removed, so they were deleted rather than kept green
against a surface that no longer exists. The figures they measured — the suffix
suite's exact CPU Q4 oracle at 32/32 with max error 1.1063e-4 — are historical for
the same reason and are not claimed here as current results. The earlier sanitizer
diagnostic was user-cancelled; no sanitizer pass is claimed.
