...
`TM_ANE=1` today: the benchmark that produced these numbers no longer compiles
against the current engine. The packer (`tools/anepack.py`) and the measurements
below survive; the engine's prefill path is the GPU/CPU hybrid. For where the lane
stopped and why, see "Where this stops" just below.
...
and is not reproduced here. The paragraph below describes that retired benchmark,
because the numbers in this document have to be readable against the CLI that
produced them.
...
anywhere — compressed-weight residency, and the benchmark's input/rejection
contracts, which skip themselves unless the benchmark binary and TinyLlama weights
are on disk — while the `test_ane*.py` group needs the CoreML/torch environment that
builds a package.
